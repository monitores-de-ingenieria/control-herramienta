...
// Reemplaza una <img> rota por su ícono de respaldo. Se llama desde
// onerror="window._fotoFallback(this)" — nada de HTML/JS embebido a mano
// en el atributo (eso fue lo que se rompió antes: comillas anidadas sin
// escapar). El ícono viaja en un atributo data-* ya escapado con
// escapeAttr(), así que aquí simplemente se lee tal cual.
window._fotoFallback = function(img) {
  const span = document.createElement("span");
  span.className = "her-foto-fallback";
  span.innerHTML = img.getAttribute("data-fallback-icono") || "";
...
window.renderEntregaPickerGrid = function() {
  ...
  wrap.innerHTML = lista.map(h => {
    ...
    const foto = fotoUrl
      ? `<img src="${escapeAttr(fotoUrl)}" data-fallback-icono="${escapeAttr(icono)}" onerror="window._fotoFallback(this)">`
...
// Número correlativo real de conduce (ej. CND-2026-00007), no el ID
// aleatorio del documento. Reutiliza la misma colección "contadores" que
// ya usan las solicitudes, pero con su propia llave para no chocar con
// esa numeración.
async function generarNumeroConduce() {
  const anio = new Date().getFullYear();
  const refContador = doc(db, "contadores", `conduces-${anio}`);
  try {
    ...
    return `CND-${anio}-${String(siguiente).padStart(5, "0")}`;
  } catch (err) {
    console.error("Error generando número de conduce:", err);
    return `CND-${anio}-${String(Date.now()).slice(-5)}`;
  }
}
...
window.confirmarNuevoPrestamoExt = async function() {
  ...
  btn.disabled = true; btn.textContent = "Guardando...";
  try {
    const numeroConduce = await generarNumeroConduce();
...
window.generarConduce = function(id) {
  ...
  // Fichas creadas antes de este cambio no tienen numeroConduce guardado
  // — para esas, se cae al ID corto como respaldo, no se rompe nada viejo.
  const numero = p.numeroConduce || `CND-${p.id.slice(0, 8).toUpperCase()}`;
  const filasHtml = (p.herramientas || []).map(h =>
    `<tr><td class="conduce-td-cant">${h.cantidad}</td><td>${escapeHtml(h.nombre)}</td></tr>`
  ).join("") || `<tr><td colspan="2" style="text-align:center;color:var(--texto-dim)">—</td></tr>`;

  document.getElementById("conduce-contenido").innerHTML = `
    <div class="conduce-doc">
      <div class="conduce-membrete">
        <div class="conduce-membrete-textos">
          <strong>Universidad Tecnológica de Santiago (UTESA)</strong>
          <span>Taller de Mecánica Industrial</span>
        </div>
        <div class="conduce-membrete-numero">
          <span class="conduce-doc-titulo">Conduce de Salida</span>
          <span class="conduce-doc-num">${escapeHtml(numero)}</span>
        </div>
      </div>

      <div class="conduce-datos">
        <div class="conduce-dato"><label>Departamento</label><div>${escapeHtml(p.departamento) || "—"}</div></div>
        <div class="conduce-dato"><label>Responsable que retira</label><div>${escapeHtml(p.responsable) || "—"}</div></div>
        <div class="conduce-dato"><label>Fecha de salida</label><div>${fecha}</div></div>
        <div class="conduce-dato"><label>Estado actual</label><div>${p.estado === "prestado" ? "Prestado" : "Devuelto"}</div></div>
      </div>

      <table class="conduce-tabla">
        <thead><tr><th class="conduce-td-cant">Cant.</th><th>Herramienta</th></tr></thead>
        <tbody>${filasHtml}</tbody>
        <tfoot><tr><td colspan="2">Total: ${totalHerConduce} herramienta${totalHerConduce === 1 ? "" : "s"}</td></tr></tfoot>
      </table>

      <div class="conduce-firmas">
        <div class="conduce-firma"><div class="conduce-firma-linea"></div><span>Entregado por<br><small>Encargado del taller</small></span></div>
        <div class="conduce-firma"><div class="conduce-firma-linea"></div><span>Recibido por<br><small>${escapeHtml(p.responsable) || "Responsable"}</small></span></div>
      </div>
    </div>
  `;
