...
// Carga una imagen desde una URL (misma carpeta img/herramientas/ que ya
// usa el fallback estático) y la comprime EXACTAMENTE igual que
// herComprimirFoto (800px máx, JPEG 88%), pero partiendo de una URL en
// vez de un File subido a mano. Se usa solo para la migración masiva.
function herComprimirFotoDesdeUrl(url) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.onload = () => {
      const MAX = 800;
    ...
    img.onerror = () => reject(new Error("No existe " + url));
    img.src = url;
  });
}

// Migración de una sola vez: recorre todas las herramientas (incluidas las
// que todavía no tienen documento propio en Firestore — las del catálogo
// de respaldo) y, para las que no tengan ya una fotoUrl propia, intenta
// traer la imagen estática de img/herramientas/{codigo}.jpg, comprimirla
// y guardarla en Firestore — así deja de depender de esa carpeta de GitHub.
window.migrarFotosDesdeGithub = async function() {
  const candidatas = (_herListaActual || []).filter(h => h.codigo && !h.fotoUrl);
  if (!candidatas.length) {
    await confirmarPersonalizado("Todas las herramientas con código ya tienen foto propia guardada — no hay nada que migrar.", true);
    return;
  }
  const ok = await confirmarPersonalizado(
    `Se va a intentar traer la foto de ${candidatas.length} herramienta(s) desde img/herramientas/ de GitHub y guardarlas en Firestore. ` +
    `Las que ya tengan foto propia no se tocan. Las que no tengan imagen en esa carpeta simplemente se saltan (avisa cuáles al final). ¿Continuar?`
  );
  if (!ok) return;
  ...
  for (const h of candidatas) {
    const url = "../img/herramientas/" + h.codigo + ".jpg";
    try {
      const dataUrl = await herComprimirFotoDesdeUrl(url);
      if (h.local) {
        // Todavía no existe como documento propio: se crea ahora, con los
        // mismos datos que ya se le mostraban desde el catálogo de respaldo.
        await addDoc(collection(db, "herramientas"), {
          nombre: h.nombre,
          cantidadDisponible: h.cantidadDisponible || 5,
        ...
        await updateDoc(doc(db, "herramientas", h.id), { fotoUrl: dataUrl });
      }
      migradas++;
    } catch (e) {
  ...
  if (migradas > 0) {
    registrarAuditoria("herramienta", "migrar-fotos", `Migró ${migradas} foto(s) desde GitHub a Firestore${fallidas.length ? ` (${fallidas.length} sin imagen en GitHub)` : ""}`);
  }

  const resumen = `Listo: ${migradas} foto(s) migrada(s).` +
    (fallidas.length ? ` ${fallidas.length} no tenían imagen en GitHub y quedaron pendientes de subir a mano: ${fallidas.join(", ")}` : " Todas se migraron.");
  await confirmarPersonalizado(resumen, true);
};
